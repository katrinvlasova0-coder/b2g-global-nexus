...
const REQUIRED_CTA =
  /leave your contacts for a consultation on tender selection and documentation preparation/i;
...
export function validateArticle(
  ...
  if (authorMatch && !/b2g\s+editorial/i.test(authorMatch[1])) {
    errors.push('❌ Author must be “B2G Editorial” (no invented experts)');
  }

  errors.push(...detectCopyPaste(body, 'EN'));

  if (content.includes('---en---')) {
    errors.push('❌ Bilingual ---en--- marker is not used in the EN-only B2G contract');
  }
  ...
  if (!REQUIRED_CTA.test(content)) {
    errors.push(
      '❌ Required CTA missing: “Leave your contacts for a consultation on tender selection and documentation preparation.”',
    );
  }
