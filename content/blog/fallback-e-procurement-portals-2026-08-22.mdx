---
title: "Electronic procurement portals: logins, timestamps, addenda"
titleEn: "Electronic procurement portals: logins, timestamps, addenda"
description: "How electronic procurement portals timestamp a bid. Educational notes on registration, file caps, timezones and official URLs — B2G is not the portal."
descriptionEn: "How electronic procurement portals timestamp a bid. Educational notes on registration, file caps, timezones and official URLs — B2G is not the portal."
...
category: "Tenders"
readTime: "8 min"
coverImage: "https://images.unsplash.com/photo-1516321318423-f06f85e504b3?w=1200&h=720&q=80&auto=format&fit=crop"
featured: false
tags: ["electronic procurement portals","Tenders","public procurement","2026"]
tagsEn: ["electronic procurement portals","Tenders","public procurement","2026"]
faq:
  - question: "What is electronic procurement portals?"
    answer: "electronic procurement portals is an educational public-procurement topic. The published notice and tender documents remain the primary source for any live procedure."
...
Anyone mapping electronic procurement portals should start with the legal or methodology source, not with marketing copy. Electronic government procurement portals are the submission channel. A bid emailed to a personal inbox is usually invalid even if it arrived on time. Readers should record the citation with article or section number and keep it next to the account contract. That shows which version applied on the as-of date. Later changes in supervisory practice are easier to spot.

## The portal is the channel, not a brochure

A second look concerns competence, the as-of date and the specific legal entity in the contract. Registration, company profile, digital certificates and user roles should be completed days before the deadline. First-time logins fail at the worst hour. An internal checklist with three fields — source, date, open question — stops call notes from replacing the file. Open questions belong with the competent body, not on forums.
...
Historically, confusion grew because brand names were mixed with statutory institutions. File-size caps, allowed extensions and virus scans reject packs that open fine on a laptop. Test uploads with dummy files where the portal allows. Cross-country comparisons help only if currency, cap logic and payout process stay separate. A table cell without a legal-basis footnote is incomplete.

In operations, written evidence outweighs oral assurances from a call centre. TED and national portals can carry the same procedure with different document packages. The notice states which package is binding. Marketing pages compress facts. Anyone preparing a decision should read the official sheet and check that the contract names the same legal entity.
...
The numbered list below is a working aid on electronic procurement portals, not a legally binding checklist.

1. Create the company profile days before the deadline.
2. Test allowed file types and size caps.
3. Record the portal timezone next to the diary date.
4. Download addenda after every publication.
5. Do not treat helpdesk chat as an amendment.
6. Bookmark the URL printed in the notice.

## TED versus national packages

The table compresses electronic procurement portals into three comparison rows. It does not replace an official information sheet.
...
![electronic procurement portals — working file](https://images.unsplash.com/photo-1504868584819-f8e8b4b35d16?w=800&q=80&auto=format&fit=crop)

## Helpdesks versus amendments

Cross-border, the competent body often changes even when the website looks the same. Timezones are explicit. A 17:00 local-time stamp is not 17:00 in the bidder’s office if the portal uses another zone. Repeated slogans in promotional emails do not change the law. What matters is the text published by the legislature, the supervisor or the scheme.

A frequent error is to treat a single slogan as a complete legal consequence. Clarifications published on the portal become part of the file. Saving only the original PDF without addenda is a common archive error. Where website, app and PDF annex disagree, the contractually incorporated text prevails. The PDF date should sit in the local archive.
...
Source work on electronic procurement portals means: directive or statute, national information sheet, and only then secondary articles. Helpdesk chats are not amendments. If an operator says “send it by email”, still check whether the documents allow that channel. Internal training should use the firm’s own account structure, not generic slides without a legal entity. Otherwise the briefing stays abstract.

Documentation rarely stops at a screenshot; contract, information sheet and date belong together. B2G does not operate official portals. Bookmark the authority’s URL from the notice, not from an ad. Auditors and tax advisers need the same data set. Separate spreadsheets with different as-of dates create later explanation work.
